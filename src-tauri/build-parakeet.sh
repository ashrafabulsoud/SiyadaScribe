...
# Compiles the patched parakeet.cpp OpenAI-compatible HTTP server as siyadascribe-whisper-server.
#
...
if [ -f "$SERVER_BIN" ]; then
    cp "$SERVER_BIN" "$SCRIPT_DIR/siyadascribe-whisper-server"
    chmod +x "$SCRIPT_DIR/siyadascribe-whisper-server"

    if [[ "$OSTYPE" == "darwin"* ]]; then
        install_name_tool -delete_rpath "$PARAKEET_DIR/build/src" "$SCRIPT_DIR/siyadascribe-whisper-server" 2>/dev/null || true
        install_name_tool -delete_rpath "$PARAKEET_DIR/build/ggml" "$SCRIPT_DIR/siyadascribe-whisper-server" 2>/dev/null || true
        ...
        if otool -L "$SCRIPT_DIR/siyadascribe-whisper-server" | grep -q "/opt/homebrew\|/usr/local/opt"; then
            echo "❌ ERROR: Binary contains Homebrew dependencies!"
            otool -L "$SCRIPT_DIR/siyadascribe-whisper-server" | grep "/opt/homebrew\|/usr/local/opt"
            exit 1
        fi
        echo "✓ No Homebrew dependencies found"
    else
        # Linux: strip build-tree rpath entries so the AppImage bundles clean deps
        patchelf --remove-rpath "$SCRIPT_DIR/siyadascribe-whisper-server" 2>/dev/null || true
        echo "siyadascribe-whisper-server binary built successfully at: $SCRIPT_DIR/siyadascribe-whisper-server"
        echo "Linked libraries:"
        ldd "$SCRIPT_DIR/siyadascribe-whisper-server" || echo "(static build, no dynamic libs)"

        if ldd "$SCRIPT_DIR/siyadascribe-whisper-server" 2>/dev/null | grep -qE "/usr/local/|/opt/"; then
            echo "❌ ERROR: Binary links against non-system paths (would break AppImage portability)!"
            ldd "$SCRIPT_DIR/siyadascribe-whisper-server" | grep -E "/usr/local/|/opt/"
            exit 1
        fi
